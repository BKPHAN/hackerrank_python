...
# 0.0
########################################################################################################################
# linalg.det : tinh dinh thuc cua mot mang
# linalg.eig : tinh gia tri rieng, vecto rieng cua 1 mang vuong
# linalg.inv : tinh nghich dao cua ma tran
########################################################################################################################
import numpy as np
...
A = []
for i in range(N):
    elt = list(map(float, input().split()))
    A.append(elt)
